...
app.post("/", async (c) => {
  ...
  const domainFilteredDocs = relevantDocs.filter((doc: any) => 
    doc.metadata && doc.metadata.domain === currentDomain
  );

  // Add context to the conversation
  const contextEnhancedMessages = [
    { role: "system", content: "You are a customer service assistant. Use the following context from the current website to help answer the user's question: " +
